...
- Encrypt the complete note with a standard tool such as [`age`](https://age-encryption.org/), using the public key. Keep the matching private key in Bitwarden, never in the repository.
...
- To unlock a note, temporarily retrieve the private key from Bitwarden: `age -d -i age-key.txt note-7f3a.age > readable-note.md`. Remove the local key and plaintext when finished.
